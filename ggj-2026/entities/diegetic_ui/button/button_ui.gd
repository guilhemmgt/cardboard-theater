...
@onready var _carton_particle: GPUParticles3D = $CartonParticle
@onready var _spot_light_3d: SpotLight3D = $SpotLight3D
@onready var _area: StaticBody3D = $ButtonBody
@onready var _label: Label3D = $Label3D

@export var text: String
@export var _mouse_speed_threshold: float = 5
...
func _process(_delta: float) -> void:
	...
	_mouse_speed = (current_mouse_pos - _mouse_pos).length()
	_mouse_pos = current_mouse_pos
	if _mouse_speed < _mouse_speed_threshold:
		_carton_particle.emitting = false
...
func _on_mouse_entered() -> void:
	var random_offset: float = randf_range(-0.2, 0.2)
	turn_with_elasticity(0.5, 3.0 + random_offset)
	_is_hover = true
	_spot_light_3d.visible = true
...
func _on_start_particle_countdown_timeout() -> void:
	_carton_particle.emitting = true
...
func _on_click(_camera: Node, event: InputEvent, _event_position: Vector3, _normal: Vector3, _shape_idx: int) -> void:
	...
	if _clicking:
		if event.is_action_released("click"):
			_clicking = false
			button_clicked.emit()
	if _is_hover and event is InputEventMouseMotion and _mouse_speed > _mouse_speed_threshold:
		_carton_particle.emitting = true
		...
		if result:
			_carton_particle.global_position = result.position - ray_dir * 0.5
